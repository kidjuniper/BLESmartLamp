...
class TimerPopUpViewController: UIViewController {
    ...
    private lazy var downView: UIView = {
        let downView = UIView(frame: CGRect(x: 0, y: 0, width: view.bounds.width * 1.1, height: view.bounds.height * 1.1))
    ...
    override func viewDidLoad() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(dismis2))
        backButton.addTarget(self, action: #selector(dismis), for: .touchUpInside)
        centerConstraint = popUpView.centerXAnchor.constraint(equalTo: view.centerXAnchor, constant: UIScreen.main.bounds.width / -1)
        downView.addGestureRecognizer(tapGesture)
        secondsPicker.selectRow(4980, inComponent: 0, animated: false)
        minutesPicker.selectRow(4980, inComponent: 0, animated: false)
        ...
        NSLayoutConstraint.activate([
                                     ...
                                     cyclesLabel.topAnchor.constraint(equalTo: minutesLabel.topAnchor),
